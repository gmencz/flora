...
const TooltipLeftOverlay = styled.div<Pick<TooltipProps, 'label'>>(
  ({ label }) => [
    tw`transform top-1/2 -translate-y-1/2 right-10 group-hover:-translate-x-1 transition-all rounded-md ease-tooltip pointer-events-none opacity-0 group-hover:opacity-100 group-hover:pointer-events-auto absolute inline-flex bg-white shadow-lg py-2 px-3 text-sm font-semibold text-gray-800`,
    label.length <= 25 ? tw`whitespace-nowrap` : tw`w-48`,
  ],
)
